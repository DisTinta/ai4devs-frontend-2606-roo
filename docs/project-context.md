...
- `PositionDetail.tsx` tracks each fetch (`getInterviewFlow`, `getCandidates`) with a per-fetch status (`loading|success|error`) (HU-5). Joint loading: one `Spinner` until **both settle**, then flow drives columns and candidates drive cards/empty/error. An empty candidates result (`[]`, success) shows a "no candidates" message with per-column placeholders and is **never** conflated with a fetch error (error shows an `Alert` + "Reintentar" that re-issues only the failed request). An unknown position id short-circuits to the not-found shell without fetching. Empty-column placeholders live inside the dnd-kit droppable so empty columns stay valid HU-4 drop targets.
- The board is responsive (HU-6): columns stack full-width below the Bootstrap `md` breakpoint (768px) and lay out horizontally with `overflow-x-auto` at `md`+ (`d-flex flex-column flex-md-row`; use `overflow-x-auto`, **not** `overflow-md-auto` — the responsive variant is not in Bootstrap's default build). dnd-kit uses `MouseSensor` (immediate) + `TouchSensor` with an activation constraint (`delay 200ms`, `tolerance 5`) so on touch a short swipe scrolls and only a long-press starts a drag. Column wrappers carry a `board-column` class (its "col" substring keeps `[class*='col']` test helpers working after moving off react-bootstrap `Col`).
