# HU-2 — Ver el proceso de contratación en columnas

**Épica:** Vista Position — Kanban de candidatos  
**Origen:** enunciado S10 (título de posición + columnas = fases del proceso)

## Historia

**Como** reclutador  
**quiero** ver el nombre de la posición y una columna por cada fase del proceso  
**para** tener contexto de dónde estoy y de cómo es el pipeline de contratación.

## Criterios de aceptación

- [ ] Se obtiene el flujo de entrevista con `GET /positions/:id/interviewFlow` (en el repo real comprobar el montaje actual, p. ej. `GET /position/:id/interviewflow`).
- [ ] El **título de la posición** (`positionName` / título equivalente en la respuesta real) se muestra en la parte superior de la vista.
- [ ] Se renderiza **una columna por cada fase** (`interviewSteps`), con su `name` como cabecera.
- [ ] Las columnas aparecen ordenadas de forma **ascendente por `orderIndex`**, no por el orden crudo del array si viniera desordenado.
- [ ] Hay tantas columnas como fases tenga el proceso (ni fijas ni inventadas en el cliente).

## Non-goals

- No mostrar aún las tarjetas de candidatos (HU-3).
- No drag & drop (HU-4).
- No cambiar el backend si la forma JSON no coincide con el ejemplo del enunciado: adaptar el mapeo en frontend o preguntar.

## Contexto técnico

- Servicio backend de referencia: `getInterviewFlowByPositionService` / ruta montada bajo `/position`.
- Tipar en frontend la respuesta real observada en red (ver riesgos de la épica sobre anidación `positionName`).

## Estimación

**S** — fetch + layout de columnas ordenadas + título.

---

<!-- The section below is an AI-generated enrichment draft. Review against the real system before accepting. -->

# [enhanced] HU-2 — See the hiring process as columns
