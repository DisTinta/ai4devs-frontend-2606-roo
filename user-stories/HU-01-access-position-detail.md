# HU-1 — Acceder al detalle de una posición

**Épica:** Vista Position — Kanban de candidatos  
**Origen:** enunciado S10 (navegación desde listado + flecha de vuelta + layout global)

## Historia

**Como** reclutador  
**quiero** pulsar "Ver proceso" en una tarjeta del listado de posiciones  
**para** abrir la vista de detalle (kanban) de esa posición.

## Criterios de aceptación

- [ ] Al pulsar "Ver proceso" en una posición, navego a la ruta de detalle incluyendo el `id` de esa posición.
- [ ] La ruta es accesible directamente por URL: recargar la página vuelve a cargar los datos de esa posición.
- [ ] Existe una flecha (o control equivalente) a la izquierda del título que devuelve al listado de posiciones (`/positions`).
- [ ] Se conserva la estructura global de la página (menú superior, pie, layout si existen); solo se crea/cambia el **contenido interno** de la vista Position.
- [ ] Los cambios de esta historia quedan en `/frontend` (routing + listado + página de detalle).

## Non-goals

- No reimplementar filtros ni tarjetas del listado de posiciones más allá de lo necesario para navegar con `id`.
- No modificar el backend.

## Contexto técnico

- Listado actual: `frontend/src/components/Positions.tsx` (mock; "Ver proceso" aún sin `Link`/`navigate`).
- Router: `frontend/src/App.js` (`react-router-dom`). Añadir ruta p. ej. `/positions/:id`.
- El listado debe exponer un `id` de posición usable en la URL (mock con id o datos de API si se acuerda).

## Estimación

**S** — navegación + shell de página con título/flecha.

---

<!-- The section below is an AI-generated enrichment draft. Review against the real system before accepting. -->

# [enhanced] HU-1 — Access a position's detail view
