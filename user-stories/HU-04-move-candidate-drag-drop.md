# HU-4 — Mover un candidato de fase arrastrando su tarjeta

**Épica:** Vista Position — Kanban de candidatos  
**Origen:** enunciado S10 (kanban con actualización de fase por drag & drop)

## Historia

**Como** reclutador  
**quiero** arrastrar la tarjeta de un candidato a otra columna  
**para** actualizar su fase en el proceso sin formularios intermedios.

## Criterios de aceptación

- [ ] Se puede arrastrar una tarjeta de una columna a otra y soltarla (ratón; táctil cubierto en HU-6).
- [ ] Al soltar en otra columna, se llama a **`PUT /candidates/:id/stage`** con body `{ applicationId, currentInterviewStep }`, donde `currentInterviewStep` es el **id** de la fase destino (columna).
- [ ] Si esa ruta **no existe** en el backend, **crearla** (mismo comportamiento que la actualización de fase actual: `:id` = candidato; body con `applicationId` y `currentInterviewStep` numérico). Se puede reutilizar el controller/servicio existentes y registrar `router.put('/:id/stage', …)` además o en lugar del `PUT /:id` genérico, según se acuerde para no romper otros usos.
- [ ] El `:id` de la ruta es el **id del candidato**; `applicationId` identifica la aplicación; `currentInterviewStep` es el **id** (`interviewSteps[].id`) de la columna destino — no el nombre.
- [ ] La tarjeta aparece en la columna destino de forma **inmediata (actualización optimista)**, sin esperar la respuesta.
- [ ] Si la petición falla, la tarjeta **vuelve** a su columna original y se muestra un **mensaje de error**.
- [ ] Soltar la tarjeta en su **misma** columna no dispara ninguna llamada.
- [ ] Mientras la petición de esa tarjeta está en curso, esa misma tarjeta no se puede volver a mover.

## Non-goals

- No editar notas ni otros campos de la aplicación.
- No reescribir la lógica de negocio de fase desde cero si ya existe: reutilizar servicio/controller y exponer la ruta `/stage`.

## Contexto técnico

- Hoy el repo tiene `PUT /candidates/:id` → `updateCandidateStageController`. Falta (o hay que añadir) `PUT /candidates/:id/stage` como pide el enunciado.
- Requiere `id` + `applicationId` del GET de candidatos (disponibles en el backend del repo).
- Resolver id de fase destino desde la columna (`interviewSteps` de HU-2).
- Elegir librería DnD en T-2 antes de implementar el frontend.

## Estimación

**M** — ruta `/stage` si falta + DnD + PUT + optimista + rollback + bloqueo durante request.

---

<!-- The section below is an AI-generated enrichment draft. Review against the real system before accepting. -->

# [enhanced] HU-4 — Move a candidate between stages by dragging its card
